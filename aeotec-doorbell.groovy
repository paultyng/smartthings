 ...
 */
 ...
 metadata {
	definition (name: "Aeon Doorbell - RV v0.5", namespace: "robertvandervoort", author: "Robert Vandervoort") {
		capability "Actuator"
		capability "Alarm"
        	capability "Battery"
		capability "Switch"
...
def btest() {
	if (state.debug) log.debug "Bell test command received"
	def request = [
...
def on() {
    ...
    if (state.notification) {
        ...
        sendEvent(name: "alarm", value: "on", type: "physical", displayed: true, isStateChange: true)
...
def off() {
    ...
    sendEvent(name: "alarm", value: "off", type: "physical", displayed: true, isStateChange: true)
